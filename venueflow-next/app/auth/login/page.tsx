...
import { ensureInitialUsers } from "@/lib/auth";
      ...
      <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70 sm:p-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Inicia sesión para administrar espacios, reservas y eventos.
          </p>
        </div>

        {params?.registered ? (
          <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">
            Cuenta creada correctamente. Ya puedes iniciar sesión.
          </div>
        ) : null}

        {params?.loggedOut ? (
          <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-semibold text-blue-800">
        ...
        <form action={loginUser} className="mt-6 grid gap-4">
          <label className="grid gap-2">
            ...
            <input
              ...
              required
              type="email"
              placeholder="admin@venueflow.local"
        ...
        <p className="mt-6 text-center text-sm text-slate-600">
          ¿No tienes cuenta?{" "}
          <Link href="/auth/register" className="font-bold text-blue-700 hover:text-blue-900">
            Regístrate
          </Link>
        </p>
      </section>
